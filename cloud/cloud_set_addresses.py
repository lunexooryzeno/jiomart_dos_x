...
def set_execution_completed():
    ...
    if response.status_code != 200:
        ...
        config_data["set_address"]["execution"] = "completed"
        
        # Now write back the updated config
        write_url = f"{BASE_URL}/api/write/json_file"
        write_payload = {
        ...
        if write_response.status_code != 200:
            return Exception(f"HTTP Error on write: {write_response.status_code}")
        elif write_response.json().get("success") != True:
            return Exception(f"API Error on write: {write_response.json().get('error')}")
        else:
            return True 
    ...
    try:
        ...
        if current_exec == required_fields.get("execution") and current_mode == required_fields.get("mode"):
            ...
            for account in selected_accs:
                ...
                try:
                    set_address_config(profile_headers_data, payload)
